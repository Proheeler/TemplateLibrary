...
public:
    ...
    auto Add(Matrix<T> const & m) const
    {
        Matrix tmp(*this);
        for(size_t i=0;i<rows_*cols_;++i)
        {
    ...
    auto Substract(Matrix<T> const & m) const
    {
        Matrix tmp(*this);
        for(size_t i=0;i<rows_*cols_;++i)
        {
    ...
    {
        assert(this->cols_ == m.getRows() && data_.size()==m.data_.size());
        Matrix tmp(*this);
        for(size_t row=0;row<rows_;++row)
        {
    ...
    auto Multiply(Matrix<T> const & m) const
    {
        assert(this->cols_ == m.getRows() && data_.size()==m.data_.size());
        Matrix tmp(*this);
        for(size_t row=0;row<rows_;++row)
